...
```mermaid
flowchart TD
    A([🧑 User Opens App]) --> B[Choose Roast Master\n8 personas available]
    B --> C{How to submit?}
    C -->|Paste text| D[📝 Paste resume\ninto textarea]
    C -->|Upload file| E[📁 Drag & drop\nTXT · DOCX · PDF]
    C -->|Use preset| F[⚡ Load sample resume\nfrom dropdown]
    D --> G[Click IGNITE THE ROAST]
    E --> G
    F --> G
    G --> H[🔄 ScanningView\nLoading animation]
    H --> I[analyzeResume\nroastEngine.js]

    subgraph ENGINE ["🔥 Roast Engine — roastEngine.js"]
        I --> J[detectContradictions\nego_inflation · devops_cosplay · api_wrapper_cosplay]
        I --> K[detectSweatInfo\nbuzzword scanning · tech claim analysis]
        I --> L[classifyAngle\n13 roast angles]
        J --> M[synthesizeCinematicRoast]
        K --> M
        L --> M
        M --> N[OPENING_POOL persona\n15 unique sentences]
        M --> O[EVIDENCE_POOL persona\n15 unique sentences]
        M --> P[PROFILE_POOL persona\n15 unique sentences]
        N --> Q[buildRoastBody\nopening + TRANSITION_A + evidence + TRANSITION_B + profile]
        O --> Q
        P --> Q
    end

    Q --> R[📊 Dashboard Rendered]

    subgraph DASHBOARD ["📊 Dashboard Sections"]
        R --> S[🎯 Verdict Card\nTitle · Paragraph Roast · Final Word]
        R --> T[📈 Sweat Diagnostics\nTutorial % · Production % · LinkedIn Delusion]
        R --> T2[🏷️ Career Archetype\nClassification badge]
        R --> U[📖 Career Lore\nFictional backstory]
        R --> V[🏆 Achievements Unlocked\nUnlocked based on detected patterns]
        R --> W[⚔️ Battle Mode Rewrites\nOriginal trash → AI Chad Version]
        R --> X[💊 Recovery Protocol\nActually useful next steps]
        R --> Y[🃏 Survivor Card\nDownloadable PNG · Copy Roast button]
    end

    S --> Z{User Action?}
    Z -->|📋 Copy Roast| AA[Clipboard → Share anywhere]
    Z -->|📥 Download Card| BB[PNG saved to device]
    Z -->|🔄 Submit Another| A
```
